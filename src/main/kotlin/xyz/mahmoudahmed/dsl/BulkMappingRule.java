...
import xyz.mahmoudahmed.dsl.util.PathMatcher.PathMatch;
import java.util.*;
import java.util.function.BiFunction;
...
public class BulkMappingRule implements MappingRule {
    ...
    public void apply(MappingContext context, Object target) {
        ...
        List<PathMatch> matches = PathMatcher.findMatches(sourceNode, sourcePattern);

        // Apply inclusions/exclusions
        List<PathMatch> filteredMatches = new ArrayList<>();
    ...
    private void applyPreservingStructure(List<PathMatch> matches, DataNode.ObjectNode targetNode) {
    ...
    private void applyFlatStructure(List<PathMatch> matches, DataNode.ObjectNode targetNode) {
